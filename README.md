...
TaPy is used for data reduction of grating interferometer data with X-rays and neutrons. It is designed for experiments with a Talbot-Lau interferometers and data that is generated by stepping a grating and thus induces intensity modulations in each pixel that are then fitted with a sinusoidal function to retrieve mean value, phase and amplitude. These paramters are then used to generate transmission images (TI), differential pahse contrast images (DPCI) and dark-field images (DFI).
...
When you execute "main.py" all required functions from "functinos.py" will be imported and are ready to use. TaPy is set up such that "main.py" can be used to sequentially call all the functions defined in "functions.py". The order can be freely modified as the output of all functions can be used as input for any function. That way you can freely order the functions you would like to execute as well as adding new ones or skipping steps by simply removing the call to the function you do not want to run.
...
# Loading data
The standard implementation TaPy ships with is a data structure where the Open Beams, the Projections and the Dark Currents are in individual folders. The folder directories have to then be specified in the main.py file. If your data structure is different TaPy allows you to write your own read-in routine to deal with any data structure.
...
We recommend following series of functions for succesful data reduction: 
 ...
 - "normalization": Select are outside of the gratings to normalise for variations in flux.
 ...
 - "cropped": Crop the image to a region of interest. This increase the computational speed.
 ...
 # Have it your way
 Feel free to develop your own functions and add the to "functions.py". If you need more complex read-in routines to fit your data structure we suggest modifying "main.py". If you think you developed things that could be interesting to the community let us know and we will incorporate it and make it available to everybody.
